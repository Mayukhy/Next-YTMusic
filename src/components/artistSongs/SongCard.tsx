...
import { setActiveSong, setActiveSongId, setActiveSongsArray, setIsplaying, setPlaylistSongData, setShowDetails, setSongMapId } from '@/redux/slices/songsSlice'
import axios from 'axios'
import { useSession } from 'next-auth/react'
import React, { useState } from 'react'
import { useDispatch, useSelector } from 'react-redux'

export default function SongCard({song,idx,suggestions,id,playlistId,addToplaylist,currentplaylistSongs}) {
    const [isShowen,setIsShowen] = useState(false)
    const {playlistSongData, activeSong,activeSongId,activeSongArr} = useSelector(state=>state.song)
    const dispatch = useDispatch()
    const fevsongs = useGetFevSongsQuery()
    ...
    const [fevSongData,setFevSongData] = useState({
      title:'',
      imgUrl:'',
      artist:'',
      id:'',
    ...
    const addFev=()=>{
      const addtoFev=async()=>{
      await axios.post(`/api/fevSongs`,fevSongData)
      .then(()=>{
       fevsongs.refetch()
      })
      }
    ...
    console.log('activeSongArray is =====>>>',activeSongArr)

    //checking that song is already added or not
    const alreadyAdded = currentplaylistSongs?.find((s)=>song?.key === s?.id)
  return (
    <div
    onClick={()=>{
      ...
      dispatch(setActiveSongsArray(suggestions?.tracks?.hits))
   ...
   { isShowen &&  <svg
    ...
    <img className={isShowen?
      'duration-200 transition-all w-[65px] h-[50px]  opacity-60':'duration-200 transition-all opacity-100  w-[65px] h-[50px]'} src={song?.images?.coverart || song?.images?.default || song?.imgUrl ||  FinalUrl} alt="" />
    </div>
    ...
    <p className=' text-zinc-300 my-auto text-start md:w-[200px] w-[100px] truncate'>{song?.subtitle || song?.heading?.subtitle || song?.attributes?.composerName || song?.artist || 'Artist name'}</p>
